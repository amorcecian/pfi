...
def getembedparam(accesstoken,idReport):
    ...
    try:
        ...
        if idReport == 1:
            REPORT_ID = app.config['REPORT_ID']
        else:
            REPORT_ID = app.config['REPORT_ID2']
        
        reporturl = 'https://api.powerbi.com/v1.0/myorg/groups/' + \
                app.config['WORKSPACE_ID'] + '/reports/' + REPORT_ID
        ...
        try:
            ...
            raise Exception(
                'Error while extracting Embed URL from API response\n' + apiresponse.text)
        ...
        if REPORT_ID != '':
            body['reports'] = []
            body['reports'].append({'id': REPORT_ID})
        ...
        try:
            ...
            apiresponse = requests.post(
                embedtokenurl, data=json.dumps(body), headers=headers)
            if app.debug:
                print('Embed token Request ID: ',
                      apiresponse.headers.get('RequestId'))
        except:
            raise Exception(
                'Error while invoking Embed token REST API endpoint\n')
        if not apiresponse:
            raise Exception('Error while retrieving report Embed URL\n' +
                            apiresponse.reason + '\nRequestId: ' + apiresponse.headers.get('RequestId'))
        ...
        try:
            apiresponse = json.loads(apiresponse.text)
            print(f"Received API Response: {apiresponse}")
